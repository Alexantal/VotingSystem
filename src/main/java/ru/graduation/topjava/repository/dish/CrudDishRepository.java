...
public interface CrudDishRepository extends JpaRepository<Dish, Integer> {
    ...
    @Query("SELECT d FROM Dish d WHERE d.restaurant.id=:rest_id " +
            "AND d.date=(SELECT MAX(dd.date) FROM Dish dd WHERE dd.restaurant.id=d.restaurant.id)")
    List<Dish> getLastDateRestaurantMenu(@Param("rest_id") int rest_id);
